---
title: "Paso 3.- Análisis de expresión diferencial (GSE31773)"
author: "Borja Gómez"
date: "20/11/2021"
output: 
  html_document:
...
```{r}
asmaticos = pData(gse31773_rma)[,"enfermedad"]

design = model.matrix(~0+asmaticos)
colnames(design) = levels(as.factor(asmaticos))
rownames(design)=colnames(exprs(gse31773_rma))
...
fit = lmFit(gse31773_rma, design)
fit1 = contrasts.fit(fit, cont.matrix_m)
fit2 = eBayes(fit1)

save(fit2, file = "data/dif_exprs_31773.rda")
```
...
write.table(final_asma,"data/genes_31773/output_asma.txt",sep = "\t", row.names = F)
write.table(final_severo,"data/genes_31773/output_severo.txt",sep = "\t", row.names = F)
write.table(final_enfermo,"data/genes_31773/output_enfermo.txt",sep = "\t", row.names = F)
```
